...
<!-- It would be useful to show an example of why we cannot necessarily determine the inclusion probabilities of edge units. The example should show how we might select the same edge unit through two or more networks. -->
...
If elements with similar target variable values tend to fall into neighborhoods (i.e., tend to be "clustered"), estimators will tend to have lower variance in comparison to a design that would only uses a random sample of as many elements. 
