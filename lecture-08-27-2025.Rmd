...
0. It is a type of **element sampling** meaning that the sampling units are the elements so that $\mathcal{U}_1 = \left\{\mathcal{E}_1\right\}, \mathcal{U}_2 = \left\{\mathcal{E}_2\right\}, \dots, \mathcal{U}_N = \left\{\mathcal{E}_N\right\}$.

0. Sampling is done *without replacement* meaning that each element can appear in the sample only once if at all. If it is possible for the same element to appear in the sample more than once then we are sampling *with replacement* (more on that later). 
