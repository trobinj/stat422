...
1. The sampling design. If there is no adjustment/re-weighting (see below) then $w_i = 1/\pi_i$ where $\pi_i$ is the inclusion probability of the $i$-th element. See [this lecture](lecture-11-04-2024.html) on inclusion probabilities.

0. Calibration. Certain estimators such as ratio and (generalized) regression estimators effectively adjust or re-weight the weights based on known totals for one or more auxiliary variables. This also includes post-stratification and raking. See [this lecture](lecture-09-13-2024.html) on how weights on re-weighting and calibration.

0. Non-response/detectability. Further adjustment or re-weighting may be done to account for known (or estimated) probabilities of response or detection. See the lectures on [detectability](lecture-11-15-2024.html) and [non-response](lecture-11-18-2024.html). 
