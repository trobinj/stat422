...
0. List all possible samples in the sampling space. For example, one possible sample is $\mathcal{S} = \{$Fredegar, Frodo, Merry, Pippin$\}$.
...
p <- 0.1
q <- 1 - p
n.est <- N*p*q*N/(N-1) / ((N-1)*(B^2/4) + p*q*N/(N-1)) 
n.bnd <- N*0.25*N/(N-1) / ((N-1)*(B^2/4) + 0.25*N/(N-1)) 
```
