...
$$
  \sqrt{V(\bar{y})} = \sqrt{\left(1 - \frac{25}{3000}\right)\frac{0.3}{25}} \approx 0.11 \ \text{cm}.
$$
...
for (i in 1:r) {    # repeatedly select samples using SRS and compute estimate
  ybar[i] <- mean(sample(anthrop$finger, n))  
}
...
Note: If you have never used R before and you'd like to try it, you can install R by downloading it from [r-project.org](https://www.r-project.org). I highly recommend that you use RStudio as your graphical user interface (GUI) to R. You can download it from [rstudio.com](https://www.rstudio.com). Both are free.
...
**Example**: Suppose we wanted to estimate the *number* and *proportion* of students at a university of 20000 students that own a mobile phone that is *not* an iPhone. To do this we use a simple random sampling design with a sample size of 100 students. In that sample 40 students report having a mobile phone that is not a iPhone. What are our estimates of the number and proportion of the students at the university that own a cell phone that is not an iPhone, and what is the bound on the error of estimation for each estimator?
...
$$
because $\bar{y}_d = \frac{1}{n_d}\sum_{i \in \mathcal{S}} y_i'$. The term $\frac{N}{n}n_d$ is effectively an *estimator* of $N_d$.
...
**Example**: Consider again the university of 20000 students and suppose we define our domain as those students that own a non-iPhone mobile phone. Again, using a simple random sampling design we select a sample of 100 students. Now suppose we want to estimate the total amount of money students in the domain spend on phone apps per year. In the sample of 100 students 40 of the students own mobile phones that are not iPhones, and we find that the mean amount of money those students spend on apps per year is 20 dollars. 
...
$$
  \hat\tau_d = N_d\bar{y} \ \ \ \text{and} \ \ \ \hat\tau_d = \frac{N}{n}n_d\bar{y}_d.
$$
...
p <- ggplot(d, aes(x = estimate, y = ..density.., fill = case)) + theme_classic()
...
p <- p + theme(legend.position = c(0.8,0.8)) + noyaxis
plot(p)
```
