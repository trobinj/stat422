...
$$
So if $\sigma_1, \sigma_2, \dots, \sigma_L$ are small, then so is $V(\hat\mu)$.

Not: Since $V(\hat\tau) = N^2V(\hat\mu)$, any strategy that reduces $V(\hat\mu)$ will also reduce $V(\hat\tau)$. 
...
0. Use *post-stratification* if $N_1, N_2, \dots, N_L$ are known, but we cannot or did not sample from the strata separately.
...
1. Obtain a larger sample of size $n'$ using simple random sampling. Determine the stratum membership of each element. Compute the number of elements in each stratum ($n_1', n_2', \dots, n_L'$). 

2. Apply stratified random sampling to the sample obtained in the first phase by applying simple random sampling to each of the sub-samples of sampled elements. 
...
```{r}
d.smp <- data.frame(Politics = c("Liberal","Moderate","Conservative"), n = c(25,15,10), m = c(5.5,4,2.25), s = c(0.9,2.1,1.1))
