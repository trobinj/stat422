...
p <- p + theme_minimal() 
p <- p + geom_point(aes(x = "Population", y = mean(mydata$y)), shape = 21, fill = "white")
p <- p + ylim(20,80) + ggtitle(" ") + xlab("") + ylab("")

p2 <- p

grid.arrange(p1, p2, widths = c(3/4, 1/4), nrow = 1)
...
p <- p + labs(color = "Stage", y = "Sample Size") + theme(legend.position = c(0.7,0.8))
...
p <- p + scale_x_continuous(name = "Between-Group Mean Square", sec.axis = sec_axis(~ ((N*M-1)*v - (N-1)*.)/(N*(M-1)), name = "Within-Group Mean Square"))
p <- p + labs(color = "Stage", y = "Sample Size") + theme(legend.position = c(0.65,0.4))
...
tmp <- solnp(pars = c(5,5), fun = vf, eqfun = cf, eqB = 100, 
  N = 25, Mbar = 50, msb = 1551.41, msw = 10.39, c1 = 10, c2 = 1, LB = c(1,1), UB = c(25,50))
tmp$pars
round(tmp$pars)
```
