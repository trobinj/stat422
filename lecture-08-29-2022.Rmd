...
A variation on simple random sampling is a design where we sample *with replacement* where very possible sample of $n$ elements has the same probability of being selected, but the elements in the sample need not be distinct. 
...
0. The inclusion probabilities when sampling *with replacement* are $\pi_i = 1 - (1 - 1/N)^n$ as opposed to $\pi_i = n/N$ when sampling *without replacement*. We will discuss how this might be used later in the course. 
...
**Example**: At the end of the previous lecture we considered an example with a population of $N$ = 50 elements and a simple random sampling design of $n$ = 5 elements. The mean and variance for the population are $\mu$ = `r m` and $\sigma^2$ $\approx$ `r round(v,2)`, respectively. What can we conclude about the sampling distribution of $\bar{y}$? 
