...
2. A "building block" for *complex* sampling designs.

A **complex sampling design** is essentially any probability sampling design other than simple random sampling, many of which we will discuss later.
...
There are many others, but we need not be concerned with the details. 
...
0. Simple random sampling is a type of **element sampling** meaning that the sampling units are the elements so that $\mathcal{U}_1 = \left\{\mathcal{E}_1\right\}, \mathcal{U}_2 = \left\{\mathcal{E}_2\right\}, \dots, \mathcal{U}_N = \left\{\mathcal{E}_N\right\}$. Thus $n$ and $N$ denote the number of *elements* in the sample and population, respectively, and also the number of *sampling units* in the sample and population, respectively.

0. Sampling is done *without replacement* meaning that each element can only appear in the sample once. Thus for, say, $n$ = 3 a sample like $\mathcal{S} = \{\mathcal{E}_1,\mathcal{E}_1,\mathcal{E}_3\}$ is not possible (i.e., it has a probability of zero). If it is possible for the same element to appear in the sample more than once then we are sampling *with replacement* (more on that later). 
...
[^inclusionprob]: To see why consider that the number of samples that include the $i$-the element equals the total number of samples minus the number of samples that *exclude* the $i$-th element, which is 
$$
...
$$
The two parameters are clearly related in that $\mu = \tau/N$ and $\tau = N\mu$.  
...
$$
where $i \in \mathcal{S}$ denotes the indices of the elements included in the sample (sometimes called an *index set*). For example, if we selected the 2nd, 3rd, and 9th elements for a sample of $n$ = 3 then $\mathcal{S} = \{2,3,9\}$ and
$$
  \bar{y} = \frac{1}{3} (y_2 + y_3 + y_9).
$$
