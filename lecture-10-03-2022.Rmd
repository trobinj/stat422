...
$$
  E[(\hat\theta - \theta)^2] = \sum_{\mathcal{S}} (\hat\theta - \theta)^2 P(\mathcal{S}).
$$
...
$$
  \hat\mu_y = \hat\mu_x\frac{\bar{y}}{\bar{x}} \ \ \ \text{or} \ \ \ 
  \hat\tau_y = \hat\tau_x\frac{\bar{y}}{\bar{x}}.
$$
\pagebreak
...
```{r, fig.height = 4}
xlab <- expression(sigma^2/sigma[r]^2)
xlab <- ifelse(knitr::is_html_output(), xlab, "$\\sigma^2/\\sigma_r^2$")
...
$$
  \hat\mu_{y,j} = \mu_{x,j}\frac{\bar{y}_j}{\bar{x}_j} \ \ \ \text{or} \ \ \ 
  \hat\tau_{y,j} = \tau_{x,j}\frac{\bar{y}_j}{\bar{x}_j},
$$
respectively. 
...
$$
  \hat\mu_y = \mu_x\frac{\bar{y}_{st}}{\bar{x}_{st}} \ \ \ \text{or} \ \ \ 
  \hat\tau_y = \tau_x\frac{\bar{y}_{st}}{\bar{x}_{st}},
$$
respectively.
...
$$
  \hat\mu_y = \frac{`r Ni[1]`}{`r N`}\underbrace{\left[`r mx[1]`\left(\frac{`r y[1]`}{`r x[1]`}\right)\right]}_{\hat\mu_{y,1}} + 
  \frac{`r Ni[2]`}{`r N`}\underbrace{\left[`r mx[2]`\left(\frac{`r y[2]`}{`r x[2]`}\right)\right]}_{\hat\mu_{y,2}} +
  \frac{`r Ni[3]`}{`r N`}\underbrace{\left[`r mx[3]`\left(\frac{`r y[3]`}{`r x[3]`}\right)\right]}_{\hat\mu_{y,3}}.
$$
The *combined* ratio estimator is 
$$
\hat\mu_y = `r sum(Ni*mx/N)`\frac{\overbrace{\left(\frac{`r Ni[1]`}{`r N`}\right)`r y[1]` + \left(\frac{`r Ni[2]`}{`r N`}\right)`r y[2]` + \left(\frac{`r Ni[3]`}{`r N`}\right)`r y[3]`}^{\bar{y}_{st}}}{\underbrace{\left(\frac{`r Ni[1]`}{`r N`}\right)`r x[1]` + \left(\frac{`r Ni[2]`}{`r N`}\right)`r x[2]` + \left(\frac{`r Ni[3]`}{`r N`}\right)`r x[3]`}_{\bar{x}_{st}}}.
$$
