...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, out.width = "100%", fig.align = "center", comment = "", cache = FALSE, dev = ifelse(knitr::is_html_output(), "png", "pdf"))
```
...
The first step is to specify the *sampling design*. But before we do that we need to add more information to the data set. Namely we need to specify the *population size* (which is 242 sections). This is done by creating a new variable.
```{r}
otters$N <- 242
head(otters)
```
...
The above shows how to make inferences concerning individual strata parameters. We can also consider (a) how to make inferences about two or more strata *combined* and also how to make inferences about *differences* between strata parameters.
...
head(otters)
```
Remember that we changed the variable `N` earlier for the stratified random sampling design. For the weights for simple random sampling $N$ = 242 and $n$ = 82. For stratified random sampling an element has a weight of $N_j/n_j$ if it is form the $j$-th stratum.
```{r}
